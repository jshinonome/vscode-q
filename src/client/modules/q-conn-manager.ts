...
class QConnManager {
    ...
    sync(query: string, queryResultHandler = this.update, tryConnect = true): void {
        if (this.isBusy && this.activeConn) {
                ...
                uniqLabel => {
                    setTimeout(() => {
                        if (this.activeConn?.uniqLabel === uniqLabel) {
                            this.sync(query, queryResultHandler, false);
                        } else {
                            queryResultHandler(
                                {
                                    type: 'error',
            ...
            queryResultHandler(
                {
                  type: 'error',
                  data: ['ERROR', 'Not connect to any process yet'],
                  duration: 0,
                  uniqLabel: 'none'
                });
        }
